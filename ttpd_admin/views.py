...
class AgenciesCreate(BaseAdminView, HasRoleMixin, LoggedCreateView):
    model = Agencies
    fields = ['name', 'address', 'phone_number', 'fax_number', 'agency_type', 'private_flag']
...
class TechnologiesUpdate(BaseAdminView, HasPermissionsMixin, LoggedUpdateView):
            ...
            try:
                with transaction.atomic():
                    ...
                    if form.has_changed():
                        ...
                        # save the industry sectors
                        if 'industry_sector_isp' in form.changed_data:
                            self.save_technology_industry_sector_isp(technology, form.cleaned_data.get('save_technology_industry_sector_isp'))
    ...
    def save_technology_industry_sector_isp(self, technology, cleaned_industry_sector_isp):
        ...
        if num_industry_sector_isp > 0:
            ...
            # clear all existing relationships first
            TechnologyIndustrySectorsISPs.objects.filter(technology=technology).delete()
        ...
        try:
            try:
                with transaction.atomic():
                    ...
                    TechnologyIndustrySectorsISPs.objects.filter(technology=self.object).delete()
